    ...
    if (editorElement) {
        editorInstance = new Editor({
            ...
            extensions: [
                ...
                BubbleMenu.configure({
                    pluginKey: 'tableBubbleMenu', // Required so it doesn't conflict with the Math menu
                ...
                BubbleMenu.configure({
                    ...
                    element: document.querySelector('#math-bubble-menu') as HTMLElement,
                    // Show this menu whenever an inline or block equation is selected
                    shouldShow: ({ editor }) => editor.isActive('inlineMath') || editor.isActive('blockMath'),
                }),

                // Simplified Mathematics Config (No more prompt popups!)
            ...
            // Listen for clicks to load the current equation into our input box
            onSelectionUpdate: ({ editor }) => {
                if (editor.isActive('inlineMath') || editor.isActive('blockMath')) {
                    const mathInput = document.getElementById('math-editor-input') as HTMLInputElement;
                    const attrs = editor.getAttributes('inlineMath');
                    const blockAttrs = editor.getAttributes('blockMath');
                    const latex = attrs.latex || blockAttrs.latex || '';

                    if (mathInput) mathInput.value = latex;
                }
            }
        });
        ...
        // Wire up the "Save" button and the "Enter" key on the new floating menu
        const updateMath = () => {
            const mathInput = document.getElementById('math-editor-input') as HTMLInputElement;
            const latex = mathInput?.value || '';

            if (window.editor.isActive('inlineMath')) {
                window.editor.chain().focus().updateInlineMath({ latex }).run();
            } else if (window.editor.isActive('blockMath')) {
                window.editor.chain().focus().updateBlockMath({ latex }).run();
            }
        };

        document.getElementById('math-update-btn')?.addEventListener('click', updateMath);
        document.getElementById('math-editor-input')?.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                e.preventDefault(); // Prevents it from adding a new line in the editor
                updateMath();
            }
        });
    }
