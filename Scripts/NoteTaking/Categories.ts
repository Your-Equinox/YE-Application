...
export function renderCategorySideBar() {
    ...
    categories.forEach(cat => {
        ...
        header.innerHTML = `
            ...
            <button class="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded px-1 transition-opacity" title="Delete Category">×</button>
        `;

        header.onclick = async (e) => {
            const target = e.target as HTMLElement;
            if (target.tagName === 'BUTTON') {
                await deleteCategoryHandler(cat.id);
                return;
            }
        ...
        header.ondrop = (e) => { header.classList.remove("bg-blue-50"); handleDrop(e, cat.id); };

        const ul = document.createElement("ul");
        ul.className = `space-y-1 mt-1 transition-all ${isCollapsed ? 'hidden' : 'block'}`;

        notes.filter(n => n.categoryID === cat.id).forEach(note => {
...
function createNoteItem(note: Note, activeId: string | null) {
    ...
    btn.draggable = true;
    btn.ondragstart = (e) => { e.dataTransfer?.setData("text/plain", note.id); btn.classList.add("opacity-40"); };
...
async function handleDrop(e: DragEvent, catId: string | null) {
    e.preventDefault();
...
async function deleteCategoryHandler(id: string) {
    ...
    // Uncategorize all notes in this category
    const notes = getNotes();
